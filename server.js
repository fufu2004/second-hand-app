...
const ADMIN_EMAIL = 'ohadf1976@gmail.com'; // הגדרת כתובת המייל של המנהל
...
app.delete('/items/:id', authMiddleware, async (req, res) => { try { const item = await Item.findById(req.params.id); if (!item) return res.status(404).json({ message: 'Item not found' }); const isAdmin = req.user.email === ADMIN_EMAIL; if (item.owner.toString() !== req.user.id && !isAdmin) return res.status(403).json({ message: 'Not authorized' }); await Item.findByIdAndDelete(req.params.id); // תיקון: שימוש בפונקציה הנכונה למחיקה io.emit('itemDeleted', req.params.id); res.json({ message: 'Item deleted' }); } catch (err) { res.status(500).json({ message: err.message }); } });
