...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
        ...
        <header class="bg-gray-100/90 dark:bg-gray-900/90 backdrop-blur-sm sticky top-0 z-40 shadow-sm dark:shadow-gray-800 py-1">
            <div class="container mx-auto px-4 flex justify-between items-center">
                ...
                <div id="header-right-desktop" class="flex-1 flex justify-start items-center gap-2">
                    <!-- PWA Install Button will be injected here -->
                ...
                <div id="header-left-desktop" class="flex-1 flex justify-end items-center gap-4">
                    <!-- Auth Content injected by JS -->
                    <div id="auth-content-desktop"></div>
                    <!-- Theme Toggle Button -->
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const reportModal = document.getElementById('report-modal'); // New
        const uploadForm = document.getElementById('upload-form');
        const ratingForm = document.getElementById('rating-form');
        const reportForm = document.getElementById('report-form'); // New
                ...
                publicProfileView.innerHTML = `
                    <div class="my-6">
                        ...
                        <div class="flex flex-col items-center gap-4 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
                            ...
                            <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">${user.displayName}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <h3 class="text-xl font-bold text-gray-700 dark:text-gray-200 mb-4">פריטים למכירה</h3>
        ...
        function createItemCard(item) {
            ...
            return `<div id="item-${item._id}" class="item-card bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-2xl overflow-hidden flex flex-col ${item.sold ? 'sold' : ''} ${item.isPromoted ? 'promoted' : ''}" data-category="${item.category || 'other'}">
                        ...
                        <div class="p-4 flex flex-col flex-grow">
                            ...
                            <div class="flex items-center my-2 gap-2"> ${categoryHtml} </div>
        ...
        async function fetchHistory() {
            ...
            try {
                ...
                itemsFeed.innerHTML = `<div class="text-center py-16 px-4 bg-white dark:bg-gray-800 rounded-lg shadow-md"><h3 class="text-lg font-medium text-red-700">לא ניתן להתחבר לשרת</h3><p class="mt-2 text-sm text-gray-600 dark:text-gray-300">לא ניתן היה ליצור קשר עם השרת. ייתכן שהשרת "מתעורר" (פעולה שלוקחת כ-30 שניות בשירותים חינמיים) או שישנה בעיית רשת.</p><p class="mt-2 text-xs text-gray-500 dark:text-gray-400">כתובת השרת שאליה מנסים להתחבר: <strong>${SERVER_URL}</strong></p><button onclick="fetchHistory()" class="mt-6 bg-teal-500 text-white font-bold py-2 px-4 rounded-md hover:bg-teal-600 transition">נסה שוב</button></div>`;
            }
        }
